"""Level 3 (Model Universe) — wraps Zhipu AI's GLM models via the BigModel
OpenAI-compatible Chat Completions endpoint as a Nexus Agent, using stdlib
`urllib` (no `zhipuai`/`openai` SDK dependency). Same shape as
`nexus.adapters.openai`/`nexus.adapters.qwen` on purpose (ARCHITECTURE.md
principle 1): this adapter only supplies GLM's own base URL, default model,
error class, and API key variable — the shared HTTP/error-handling/
...
DEFAULT_MODEL = "glm-4-flash"  # free/cheapest current model — a sane default for a generic adapter
...
def call_glm(
    ...
    return post_chat_completion(
        ...
        max_tokens=max_tokens, api_key=api_key, vendor="GLM", error_cls=GLMError,
    )
...
def make_glm_agent(
    ...
    falls back to `GLM_API_KEY` (a Zhipu BigModel API key), resolved once at
    agent-construction time — identical contract to
    `nexus.adapters.openai.make_openai_agent`."""
    resolved_key = api_key or os.environ.get("GLM_API_KEY")
    ...
    def handle(task: Task) -> dict[str, Any] | ErrorPayload:
        if not resolved_key:
            return agent.fail(
                ...
                message="GLM_API_KEY is not set and no api_key was provided", retryable=False,
            )
        ...
        try:
            response = call_glm(prompt, api_key=resolved_key, model=model, api_url=api_url)
        except GLMError as exc:
            return agent.fail(task, error_code="glm_api_error", message=str(exc), retryable=True)
        return {
