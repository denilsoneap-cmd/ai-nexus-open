"""Level 3 (Model Universe) — wraps Tencent's Hunyuan models via their
...
principle 1): this adapter only supplies Hunyuan's own base URL, default
...
DEFAULT_MODEL = "hunyuan-lite"  # free/cheapest current model — a sane default for a generic adapter
...
def call_hunyuan(
    ...
    max_tokens: int = 1024,
    api_url: str = HUNYUAN_API_URL,
    timeout: float = 60.0,
) -> dict[str, Any]:
    return post_chat_completion(
        prompt=prompt, model=model, api_url=api_url, timeout=timeout,
        max_tokens=max_tokens, api_key=api_key, vendor="Hunyuan", error_cls=HunyuanError,
    )


def make_hunyuan_agent(
    name: str,
    ...
    falls back to `HUNYUAN_API_KEY`, resolved once at agent-construction
    time — identical contract to `nexus.adapters.openai.make_openai_agent`."""
    resolved_key = api_key or os.environ.get("HUNYUAN_API_KEY")
    ...
    def handle(task: Task) -> dict[str, Any] | ErrorPayload:
        if not resolved_key:
            return agent.fail(
                ...
                message="HUNYUAN_API_KEY is not set and no api_key was provided", retryable=False,
            )
        ...
        try:
            response = call_hunyuan(prompt, api_key=resolved_key, model=model, api_url=api_url)
        except HunyuanError as exc:
            return agent.fail(task, error_code="hunyuan_api_error", message=str(exc), retryable=True)
        return {
