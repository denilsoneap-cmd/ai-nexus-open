"""Level 3 (Model Universe) — wraps Baidu's ERNIE models via the Qianfan V2
OpenAI-compatible Chat Completions endpoint as a Nexus Agent, using stdlib
`urllib` (no `qianfan`/`openai` SDK dependency). Same shape as
`nexus.adapters.openai`/`nexus.adapters.qwen` on purpose (ARCHITECTURE.md
principle 1): this adapter only supplies ERNIE's own base URL, default
...
ERNIE_API_URL = "https://qianfan.baidubce.com/v2/chat/completions"
DEFAULT_MODEL = "ernie-speed-128k"  # cheapest current model — a sane default for a generic adapter


class ErnieError(RuntimeError):
    pass
...
def call_ernie(
    ...
    max_tokens: int = 1024,
    api_url: str = ERNIE_API_URL,
    timeout: float = 60.0,
) -> dict[str, Any]:
    return post_chat_completion(
        prompt=prompt, model=model, api_url=api_url, timeout=timeout,
        max_tokens=max_tokens, api_key=api_key, vendor="ERNIE", error_cls=ErnieError,
    )


def make_ernie_agent(
    name: str,
    ...
    falls back to `ERNIE_API_KEY` (a Qianfan V2 API key), resolved once at
    agent-construction time — identical contract to
    `nexus.adapters.openai.make_openai_agent`."""
    resolved_key = api_key or os.environ.get("ERNIE_API_KEY")
    ...
    def handle(task: Task) -> dict[str, Any] | ErrorPayload:
        if not resolved_key:
            return agent.fail(
                ...
                message="ERNIE_API_KEY is not set and no api_key was provided", retryable=False,
            )
        ...
        try:
            response = call_ernie(prompt, api_key=resolved_key, model=model, api_url=api_url)
        except ErnieError as exc:
            return agent.fail(task, error_code="ernie_api_error", message=str(exc), retryable=True)
        return {
